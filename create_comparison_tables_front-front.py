...
for modelname in models:
	...
	for i in range(1,10):
		print(DeepFace.verify(img1_path = "faces/original/"+str(i)+"/5.jpg", img2_path = "faces/original/1/5.jpg", detector_backend='skip', model=model)['distance'], end = "\r")
	...
	for comparisontype in comparisontypes:
		...
		for i in range(1,31):
			for j in range(1,31):
				...
				resultsarray[i-1,j-1] = DeepFace.verify(img1_path = "faces/original/"+str(i)+"/5.jpg", img2_path = "faces/original/"+str(j)+"/5.jpg", detector_backend='skip', model=model)['distance']
		...
		for h in options:
			...
			for i in range(1,31):
				for j in range(1,31):
					...
					resultsarray[i-1,j-1] = DeepFace.verify(img1_path = "faces/original/"+str(i)+"/5.jpg", img2_path = 'faces/'+comparisontype+"/"+comparisontype+"_"+str(h)+"/"+str(j)+"/5.jpg", detector_backend='skip', model=model,)['distance']
			results.append(resultsarray)
		...
		save('saved_comparison_data/'+modelname+'/5-5/'+comparisontype+'/original_vs_'+comparisontype+'s.npy', data)
		print("Data saved!")
		...
		for y in options:
			...
			for i in range(1,31):
				for j in range(1,31):
					#print(str(i) + " " + str(j), end = "\r")
					resultsarray[i-1,j-1] = DeepFace.verify(img1_path = 'faces/'+comparisontype+"/"+comparisontype+"_"+str(y)+"/"+str(i)+"/5.jpg", img2_path = "faces/original/"+str(j)+"/5.jpg", detector_backend='skip', model=model)['distance']
			results.append(resultsarray)
			...
			for h in options:
				...
				for i in range(1,31):
					for j in range(1,31):
						#print(str(h) + " " + str(i) + " " + str(j), end = "\r")
						resultsarray[i-1,j-1] = DeepFace.verify(img1_path = 'faces/'+comparisontype+"/"+comparisontype+"_"+str(y)+"/"+str(i)+"/5.jpg", img2_path = 'faces/'+comparisontype+"/"+comparisontype+"_"+str(h)+"/"+str(j)+"/5.jpg", detector_backend='skip', model=model)['distance']
				results.append(resultsarray)
				print(comparisontype + " level " + str(h) + " done.")
				totaltime = time.perf_counter() - starttime2
				print("Execution time: " + str(time.strftime('%H:%M:%S', time.gmtime(totaltime))))
			...
			#Save results to datafile for further processing
			data = asarray(results)
			save('saved_comparison_data/'+modelname+'/5-5/'+comparisontype+'/'+comparisontype+'_'+str(y)+'_vs_'+comparisontype+'s.npy', data)
			print("Data saved!")
	...
	print("All comparison types done! This was model: " + modelname+ ". Total final execution time: " + str(time.strftime('%H:%M:%S', time.gmtime(totaltime))))
	totaltime = time.perf_counter() - super_starttime
	print("Time elapsed so far: " + str(time.strftime('%H:%M:%S', time.gmtime(totaltime))))

print("Finally done! Time it took: " + str(time.strftime('%H:%M:%S', time.gmtime(totaltime))))
