...
#instead of doing just a single face angle comparison. This script still contains a lot of #print statements that probably just are not necessary anymore. I have disabled most of them.
import os
import time
...
for reference_angle in range(1,10):
	for test_angle in range(1,10):
		print(str(reference_angle)+'-'+str(test_angle), end='\r')
		#create all necessary directories if they don't exist
		directory = 'VGG-Face2'
		current_dir = os.getcwd()
		req_dir = current_dir+'/saved_comparison_data/'+directory+'/'+str(reference_angle)+'-'+str(test_angle)
		...
		for comparisontype in comparisontypes:
			...
			##print('Reading reference ('+str(reference_angle)+') originals...', end='\r')
			imgs = []
			for face_number in range(1,31):
				img = Image.open('faces/original/'+str(face_number)+'/'+str(reference_angle)+'.jpg')
				face_array = asarray(img)
				imgs.append(face_array)

			for option in options:
				##print('Reading reference ('+str(reference_angle)+') '+comparisontype+' '+str(option)+'...', end='\r')
				for face_number in range(1,31):
					img = Image.open('faces/'+comparisontype+'/'+comparisontype+'_'+str(option)+'/'+str(face_number)+'/'+str(reference_angle)+'.jpg')
					face_array = asarray(img)
					imgs.append(face_array)

			#now calculate the embeddings of the images in memory
			#print('Calculating reference_angle ('+str(reference_angle)+') embeddings...')
			samples = asarray(imgs, 'float32')
			samples = preprocess_input(samples, version=2)
			reference_face_embeddings = model.predict(samples)
			#print('Reference embeddings done!')

			#do the same thing again, but for the test face images
			#print('Reading test ('+str(test_angle)+') originals...', end='\r')
			imgs = []
			for face_number in range(1,31):
				img = Image.open('faces/original/'+str(face_number)+'/'+str(test_angle)+'.jpg')
				face_array = asarray(img)
				imgs.append(face_array)

			for option in options:
				#print('Reading test ('+str(test_angle)+') '+comparisontype+' '+str(option)+'...', end='\r')
				for face_number in range(1,31):
					img = Image.open('faces/'+comparisontype+'/'+comparisontype+'_'+str(option)+'/'+str(face_number)+'/'+str(test_angle)+'.jpg')
					face_array = asarray(img)
					imgs.append(face_array)

			#print('Calculating test ('+str(test_angle)+') embeddings...')
			samples = asarray(imgs, 'float32')
			samples = preprocess_input(samples, version=2)
			test_face_embeddings = model.predict(samples)
			...
			for i in range(0,30):
				for j in range(0,30):
					resultsarray[i,j] = cosine(reference_face_embeddings[i], test_face_embeddings[j])
			...
			for test_option_number, option in enumerate(options):
				...
				for i in range(0,30):
					for j in range(0,30):
						resultsarray[i,j] = cosine(reference_face_embeddings[i], test_face_embeddings[j+(test_option_number+1)*30])
				results.append(resultsarray)
			...
			save('saved_comparison_data/'+modelname+'/'+str(reference_angle)+'-'+str(test_angle)+'/'+comparisontype+'/original_vs_'+comparisontype+'s.npy', data)
			#print('Data saved!')
			...
			for reference_option_number, reference_option in enumerate(options):
				...
				for i in range(0,30):
					for j in range(0,30):
						resultsarray[i,j] = cosine(reference_face_embeddings[i+(reference_option_number+1)*30], test_face_embeddings[j])
				results.append(resultsarray)
				...
				for test_option_number, test_option in enumerate(options):
					...
					for i in range(0,30):
						for j in range(0,30):
							resultsarray[i,j] = cosine(reference_face_embeddings[i+(reference_option_number+1)*30], test_face_embeddings[j+(test_option_number+1)*30])
					results.append(resultsarray)
				...
				#save results to datafile for further processing
				data = asarray(results)
				save('saved_comparison_data/'+modelname+'/'+str(reference_angle)+'-'+str(test_angle)+'/'+comparisontype+'/'+comparisontype+'_'+str(reference_option)+'_vs_'+comparisontype+'s.npy', data)
				#print('Data saved!')
