...
for comparisontype in comparisontypes:
	...
	print("Reading frontal originals...", end='\r')
	imgs = []
	for i in range(1,31):
		img = Image.open("faces/original/"+str(i)+"/5.jpg")
		face_array = asarray(img)
	...
	for option in options:
		print("Reading frontal "+comparisontype+" "+str(option)+"...", end='\r')
		for i in range(1,31):
			img = Image.open("faces/"+comparisontype+"/"+comparisontype+"_"+str(option)+"/"+str(i)+"/5.jpg")
			face_array = asarray(img)
			imgs.append(face_array)

	print("Calculating frontal embeddings...")
	samples = asarray(imgs, 'float32')
	samples = preprocess_input(samples, version=2)
	front_face_embeddings = model.predict(samples)
	print("Frontal embeddings done!")

	print("Reading left-facing originals...", end='\r')
	imgs = []
	for i in range(1,31):
	...
	for option in options:
		print("Reading left-facing "+comparisontype+" "+str(option)+"...", end='\r')
		for i in range(1,31):
			img = Image.open("faces/"+comparisontype+"/"+comparisontype+"_"+str(option)+"/"+str(i)+"/1.jpg")
			face_array = asarray(img)
			imgs.append(face_array)

	print("Calculating left-facing embeddings...")
	samples = asarray(imgs, 'float32')
	samples = preprocess_input(samples, version=2)
	left_face_embeddings = model.predict(samples)
	print("Left-facing embeddings done!")
	...
	for reference_option_number, reference_option in enumerate(options):
		...
		#Save results to datafile for further processing
		data = asarray(results)
		save('saved_comparison_data/'+modelname+'/'+comparisontype+'/'+comparisontype+'_'+str(reference_option)+'_vs_'+comparisontype+'s.npy', data)
		print("Data saved!")
